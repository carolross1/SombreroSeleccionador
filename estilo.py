...
def tooltip_html(texto: str, explicacion: str) -> str:
    """Envuelve un termino tecnico en un span con tooltip nativo del
    navegador (title=), para explicarlo sin saturar el texto visible."""
    return (
        f'<span title="{explicacion}" '
        f'style="border-bottom: 1px dotted var(--dorado-claro); cursor: help;">{texto}</span>'
    )
...
.hp-candela {
    ...
    background: radial-gradient(circle at 50% 30%, #fff2c9 0%, #ffb84d 45%, #c9781f 100%);
...
def candelas_html() -> str:
    ...
    velas = "".join(
        f'<span class="hp-candela" style="left:{pos}%; animation-delay:{delay}s;"></span>'
...
def badge_casa_html(casa: str) -> str:
    color = logica.COLORES.get(casa, "#333")
    return f'<span class="hp-badge" style="background:{color};">{casa}</span>'
...
def veredicto_html(casa: str, subtitulo: str = "") -> str:
    ...
    <div class="hp-veredicto" style="background: radial-gradient(circle at top, {color}, #100c08 85%);">
        ...
        <div class="titulo" style="color:#f4ecd8;">¡{casa.upper()}!</div>
